...
"GROUP BY PCR.NombreClasificacionRiesgo ORDER BY PCR.NombreClasificacionRiesgo");*/
                ...
                if(opc == 3)
                {
                    strConsulta = string.Format("SELECT RR.IdClasificacionRiesgo,PCR.NombreClasificacionRiesgo,COUNT(RR.IdRiesgo) as cant,ISNULL(RR.CodRiesgo,'') AS CodRiesgo,RCtrl.IdControl,PRI.IdProbabilidad,PRI.IdImpacto,LTRIM(RTRIM(ISNULL(RCtrl.efectividad, ''))) efectividad,LTRIM(RTRIM(ISNULL(RCtrl.operatividad, ''))) operatividad , (SELECT count(1)" +
